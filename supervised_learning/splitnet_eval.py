...
NUM_TEST_BATCHES_PER_EPOCH = 1000
...
def evaluate_model(
    model, device, test_loader, total_num_steps, logger, net_output_info, checkpoint_dir
):
    model.eval()
    loss_val_total = 0
    object_loss_total = 0
...
def main():
    ...
    folder = os.path.join(log_prefix, checkpoint_dir)
    start_iter = pt_util.restore_from_folder(model, folder)
    print("folder: ", folder)
