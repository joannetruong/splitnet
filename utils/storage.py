...
class RolloutStorageWithMultipleObservations(RolloutStorage):
    ...
    def to(self, device):
        ...
        for key in self.additional_observations_dict:
            self.additional_observations_dict[key] = self.additional_observations_dict[
                key
            ].to(device)
        ...
        for key in self.additional_observations_dict:
            if key not in obs:
                ...
                if key not in self._warn_once["copy_obs"]:
                    print(
                        "Warning! Key", key, "not in observation in function copy_obs"
                    )
        ...
        for key in self.additional_observations_dict:
            if key not in seq_dict:
                ...
                if key not in self._warn_once["copy_seq"]:
                    print(
                        "Warning! Key", key, "not in observation in function copy_seq"
                    )
    ...
    def after_update(self):
        for key in self.additional_observations_dict:
            self.additional_observations_dict[key][0].copy_(
                self.additional_observations_dict[key][-1]
            )
    ...
    def remove_worker_storage(self, index):
        with torch.no_grad():
            ...
            self.recurrent_hidden_states = torch.cat(
                (
                    self.recurrent_hidden_states[:, :index],
                    self.recurrent_hidden_states[:, index + 1 :],
                ),
                dim=1,
            )
            self.rewards = torch.cat(
                (self.rewards[:, :index], self.rewards[:, index + 1 :]), dim=1
            )
            self.value_preds = torch.cat(
                (self.value_preds[:, :index], self.value_preds[:, index + 1 :]), dim=1
            )
            self.returns = torch.cat(
                (self.returns[:, :index], self.returns[:, index + 1 :]), dim=1
            )
            self.action_log_probs = torch.cat(
                (
                    self.action_log_probs[:, :index],
                    self.action_log_probs[:, index + 1 :],
                ),
                dim=1,
            )
            self.actions = torch.cat(
                (self.actions[:, :index], self.actions[:, index + 1 :]), dim=1
            )
            self.masks = torch.cat(
                (self.masks[:, :index], self.masks[:, index + 1 :]), dim=1
            )
        ...
        if mini_batch_size is None:
            assert batch_size >= num_mini_batch, (
                ...
                "".format(
                    num_processes,
                    num_forward_rollout_steps,
                    num_processes * num_forward_rollout_steps,
                    num_mini_batch,
                )
            )
    ...
    def recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            additional_obs_batch = {
                key: torch.stack(
                    [additional_obs_batch[ii][key] for ii in range(num_envs_per_batch)],
                    1,
                )
            ...
            additional_obs_batch = {
                key: pt_util.remove_dim(val, 1)
                for key, val in additional_obs_batch.items()
            }
            ...
            old_action_log_probs_batch = pt_util.remove_dim(
